...
| 角色 | 目标职责 | 当前实现 | 必须补齐 |
| --- | --- | --- | --- |
| 小吱 | 主控，按金谷与盈蓄状态持续输出 | 已改为 monotonic 有界大招循环，具备换人/死亡/脱战检查；普通与大招窗口均支持最多三次 E | 实机校准金谷颜色采样与防抖；确认三段 E 的实际命中率与动画间隔 |
...
```text
九原 E -> [零 Q] -> 零 E -> 立即回小吱 Q/三段 E/持续站场
```

首轮只负责建立九原与零的铺垫，不在零场上等待第二次 E。route 完成后回到小吱，由小吱原生
入场流程负责 Q、三段 E 与持续输出。

后续循环路线：

```text
小吱站场等待环合 -> 环合切九原(创生) -> [九原 Q]
                 -> [零 Q] -> 零 E -> 环合切翳(延滞)
                 -> [翳 Q] -> 翳 E -> [九原 E 聚怪] -> 回小吱爆发
```

路线明确要求两个真实环合事件，不能把同为光属性的 `零 -> 小吱` 伪装成环合。第一个环合从
小吱站场自然触发，零只在第二段铺垫时短切释放 Q/E；任何必需动作在冷却中都由 Planner 有界
等待并使用普通攻击 fallback，不能高频空放技能。
...
- 每轮技能链最多尝试三次 E。大招期间继续使用金谷颜色门控；非大招期间使用两次平 A 后一次 E
  的保守规则，并在每段之间重新检查当前角色、死亡、脱战与技能可用状态。
...
- 小吱首轮改为九原 E、零可选 Q/E 后立即回小吱，不再让零站场等待第二次 E。
- 小吱普通技能链和大招循环均支持最多三次 E；普通技能链每段前两次平 A，大招循环继续使用
  金谷颜色门控，并增加换人、死亡、脱战与技能可用检查。
- 小吱站场等待第一次创生环合；随后执行九原、零、翳短切和第二次延滞环合，最后回小吱。
- 零的普通 Q/E 增加可用性门控，冷却中不再高频重复提交动作；日志分析器会把零技能重试风暴
  和长时间零站场等待直接判为异常。
