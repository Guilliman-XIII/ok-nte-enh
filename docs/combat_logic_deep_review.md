...
- 小吱普通与大招技能链均支持最多三次 E；每次 E 前穿插 0.5 秒普攻，并逐段检查角色、技能、
  死亡和脱战状态。
- 小吱先站场输出，随后以 `小吱 -> 九原` 触发创生；零与翳完成第二段延滞铺垫后再回小吱。
- 零 Q/E 增加技能可用性门控，日志分析器新增零技能重试风暴与长时间站场等待诊断。
- 项目规定的完整测试命令通过 `400` 项测试，目标队伍轴、三段 E、中断边界和日志异常均有回归覆盖。

当前 Go/No-Go：**白藏队低压 Go、高压 Conditional Go；小吱队实机 No-Go、定向复测 Go**。
