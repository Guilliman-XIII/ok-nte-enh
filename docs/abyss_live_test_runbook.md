...
- 深渊自动双队是显式开关。新战斗只有当前 HUD 的四个头像完整、高置信且唯一匹配已保存预设时才
  进入严格深渊绑定；不匹配的新战斗保留 OKNTE 原有的通用角色识别。已绑定的深渊会话中，部分识别、
  歧义、缺少特征或队伍不完整都会停止输入。
- 上下半场交接时先暂停旧队输入，等待第二个一致完整帧后才重建队伍和 Planner。手动“用于下一场”模式
  仍可使用，但发现实际队伍变化时只会停手，不会继续沿用上一队逻辑。
