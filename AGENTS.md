...
优先级：**稳定 > 高效自动化 > 简洁 > 美观**。冲突时高优先级胜出。

五条不可违反的规则：

1. **方向键 `try/finally`**：所有 `send_key_down` 必须在 `finally` 中释放。
2. **长循环分片检查**：>0.5 秒的循环每分片检查 `is_current_char`、`is_dead`、`check_combat()`、`deadline`。禁止 `while True`。
3. **Slot 语义真实**：`ActionIntent.slot` 必须反映实际消耗的资源。burst 内调用其他 slot 的技能必须先 `context.can_execute_action(self, slot=...)`。
4. **不伪造成功**：兜底动作返回值必须真实，`priority_ready=False` 不吸引切人。
5. **时钟用 `time.monotonic()`**：通过 `_now()` 包装，测试只 patch `_now()`。

其他约定：`combat_start_priority` 默认 0；爆发窗口内 `click_skill` 用短超时（2s）；类 docstring 不写未验证机制（放 `docs/research/`）；日志前缀 `[角色名]`；第二 E 用三态 `disabled|observe|execute`，默认 observe。

测试覆盖：Factory → Role → CombatPlan → 状态机 → 输入安全 → Planner 集成。Mock 只覆盖输入方法，不覆盖决策逻辑。
